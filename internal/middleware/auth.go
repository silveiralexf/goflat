...
// registers on pocketbase middleware that
// Sets and Reads session data into a secure cookie
func AddCookieSessionMiddleware(pb *pocketbase.PocketBase) {
	pb.OnBeforeServe().Add(func(e *core.ServeEvent) error {
		e.Router.Use(loadAuthContextFromCookie(pb))
		return nil
	})
	...
	pb.OnAdminAuthRequest().Add(func(e *core.AdminAuthEvent) error {
		e.HttpContext.SetCookie(&http.Cookie{
			Name:     AuthCookieName,
			Value:    e.Token,
			Path:     "/",
	...
	pb.OnBeforeServe().Add(getLogoutRoute(pb))
}

func loadAuthContextFromCookie(coreApp core.App) echo.MiddlewareFunc {
	return func(next echo.HandlerFunc) echo.HandlerFunc {
		return func(c echo.Context) error {
			...
			case tokens.TypeAdmin:
				admin, err := coreApp.Dao().FindAdminByToken(
					token,
					coreApp.Settings().AdminAuthToken.Secret,
				)
			...
			case tokens.TypeAuthRecord:
				record, err := coreApp.Dao().FindAuthRecordByToken(
					token,
					coreApp.Settings().RecordAuthToken.Secret,
				)
...
func getLogoutRoute(_ *pocketbase.PocketBase) func(*core.ServeEvent) error {
